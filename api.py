...
class PetFriends:
    ...
    def create_pet_simple(self, auth_key: json, name: str, animal_type: str,
                          age: str) -> json:
        """Метод отправляет POST-запрос с данными о новом добавляемом питомце без фотографии и возвращает статус
        запроса на сервер и результат в формате JSON с данными добавленного питомца"""
    ...
    def add_pet_photo(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Метод отправляет POST-запрос на о добавлении фотографии по указанному pet_id существующего питомца
